...
### 模块（可在设置中开启/关闭、配置常驻或切换、上下箭头调整前后顺序）
| 模块 | 说明 |
|---|---|
...
| 自定义 | 用户添加 shell 命令小部件，两种展示方式：**文本**（输出原样显示）与**热力图**（输出数字按大小着色，命令失败/超时显示红色方格，每 30 秒后台探测）。设置页内置**示例模板**一键添加（本机 IP、服务响应速度探测、磁盘占用、CPU 前几名、先登录再取数等），完整写法见 [docs/custom-items.md](docs/custom-items.md) |
